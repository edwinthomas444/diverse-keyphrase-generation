...
class BertEncoder(nn.Module):
    def __init__(self, config):
        super().__init__()
        self.config = config
        self.layer = nn.ModuleList([BertGenerationLayer(
...
class BertGenerationEncoder(BertGenerationPreTrainedModel):
    ...
    def forward(
        ...
        if self.config.is_decoder and encoder_hidden_states is not None:
            # print('Encoder hidden states size: ', encoder_hidden_states.size())
...
class BertGenerationDecoder(BertGenerationPreTrainedModel):
    _keys_to_ignore_on_load_missing = [
        "lm_head.decoder.weight", "lm_head.decoder.bias", "embeddings.position_ids"]
    ...
    # add rnn head
    def add_tiny_rnn_head(self, max_units):
        # innitialize a RNN layer
        self.tiny_rnn_layers = torch.nn.ModuleList([torch.nn.RNN(input_size=768,
                                                                 hidden_size=768,
                                                                 num_layers=2,
                                                                 nonlinearity='tanh',
                                                                 batch_first=True,
                                                                 bidirectional=False) for i in range(max_units)])
    ...
    def forward(
        ...
        # model dependancies using a RNN layer
        if self.config.rnn_head:
            rnn_inp = sequence_output.contiguous().view(-1, max_kps, max_kp_len, 768)

            sequence_output_rnn = []
            for i in range(max_kps):
                unit_inp = rnn_inp[:, i, :, :]
                unit_out, _ = self.tiny_rnn_layers[i](unit_inp)
                sequence_output_rnn.append(unit_out.unsqueeze(1))
            sequence_output = torch.cat(
                sequence_output_rnn, dim=1).contiguous().view(-1, max_kps*max_kp_len, 768)

        prediction_scores = self.lm_head(sequence_output)
        curr_device = inputs_embeds.device if inputs_embeds is not None else input_ids.device

        lm_loss = None
        if labels is not None:
            # do not shift labels or outputs
            ...
            active_loss_indices = (
                labels.view(-1) != 0) & (labels.view(-1) != 101) & (labels.view(-1) != 102) & (labels.view(-1) != 100)
            ...
            # print('active loss indices: ',active_loss_indices)
            ...
            # # a batch is penalized only for 1 effective none token
            none_wt = min(1.0, 1.0/(norm_factor+1e-08))
            # # print('active labels: ', active_labels)
            # # print('none token weight: ', none_wt,
            # #       'num none tokens: ', none_count.item())
